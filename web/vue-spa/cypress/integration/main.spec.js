...
it("While img is editing buttons inactive ", () => {
});
...
// it("Clicking buon save", () => {
//   let getStore ='';
//   cy.window().then(win => {
//     getStore = () => win.its('app.$store');
//   })
//   // const getStore = () => cy.window().its('app.$store');
//   cy.get('.effect-text').should(($effect_text) => {

//     expect(getStore().its('state.activeFilter')).to.equal('Черно-белое');
//     const text = $effect_text.text();
//     expect(text).to.equal('Эффект: Черно-белое');
//     // expect(getStore().its('state.activeFilter')).to.equal(text);
//     // getStore().its('state.activeFilter').should('match', effect);
//     // expect(text).to.equal(' Применить к изображению выбранный эффект? ');
//   })
//   // cy.get('.effect-text').should(($effect_text) => {
//   //   const effect = $effect_text.text();
//   //   getStore().its('state.activeFilter').should('match', effect);
//   // });
// });